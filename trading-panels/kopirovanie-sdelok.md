...
{% hint style="info" %}
**Этот параметр задает соотношение количества \(объема\) сделки дочернего счета \(коннекта\) для ордеров по отношению к родительскому счету \(коннекту\) с учетом коэффициэта или %.**
{% endhint %}
